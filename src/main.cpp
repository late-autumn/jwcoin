...
#if defined(NDEBUG)
# error "Jwcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0b81cfb442a9ac27730c9772429af5bcec83dbec751d7706384720a1a207b342");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jwcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jwcoin Signed Message:\n";
...
{
    // Jwcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Jwcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Jwcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Jwcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jwcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jwcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x28e6b1ebce091355446c6f7a3d7ccd0ecf7238d34e754342e9f2610600e5b662");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is JW coin. Timestamp Bye ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1526629275;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086322451;

        if (fTestNet)
        {
            block.nTime    = 1526629256;
            block.nNonce   = 386827175;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x11ddd411a85fb92af87ce647683fae047220928d9bbf422966431a50523310bd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc3, 0xba, 0xd2 }; // Jwcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JwcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JwcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JwcoinMiner(CWallet *pwallet)
{
    printf("JwcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jwcoin-miner");
    ...
    try { loop {
        ...
        printf("Running JwcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JwcoinMiner, pwallet));
}
